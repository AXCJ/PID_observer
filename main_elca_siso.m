...
D = zeros(p,m);
disp([num2str(m) '-IN ' num2str(p) '-OUT']);

% Zeros_ori = tzero(A, B, C, D);
Zeros_ori = TZOCS(A, B, C, D)
% disp(num2str(Zeros_ori));
...
if Rc_ori ~= size(A,1) || Ro_ori ~= size(A,1)
...
C_tilde = eta * C;
D_tilde = eta * D;
% Zeros_tilde = tzero(A, B, C_tilde, D_tilde);
...
% ~~~ Check whether the observer is controllable and observable
...
for ts = 0 : dt : T_stop
    ...
    ref_c(:,i) = ref_func(ts,p);  % reference 
...
figure % y_xi vs. ye_xi
signal = y_xi;
size_fig = min(size(signal));
for i = 1:size_fig
subplot(size_fig*100+10+i)
plot(ts_c, y_xi(:,i), 'b-', ts_c, ye_xi(:,i)', 'r:', 'LineWidth', 1.5)
ylabel(['\fontsize{10} y_{\xi' num2str(i) '} vs. y_{\xie' num2str(i) '}'])
legend(['\fontsize{10} y_{\xi' num2str(i) '}'],['\fontsize{10} y_{\xie' num2str(i) '}'],'Location','NorthWest','Orientation','Horizontal')
end
xlabel('\fontsize{10} Time ( sec. )')
% axis([0 T_stop -120 170])
